...
router.get(["/supervision/:periodo", "/api/supervision/:periodo"], async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        -- =================================================================
        -- 1. AGENCIA COMPLETA (Todos los asesores)
        -- =================================================================
        WITH CTE_Flow_All AS (
        ...
        CTE_MetasStock_All AS (
            SELECT 
                ...
                ISNULL((
                    ...
                    WHERE DA.IdSAgencia = M.IdSAgencia AND A.Periodo = @periodo
                ), 0) AS CarteraInicial
        ...
        -- =================================================================
        -- 2. AGENCIA COMERCIAL (Solo Analistas y Admins)
        -- =================================================================
        ...
        CTE_MetasStock_Com AS (
            SELECT 
                ...
                ISNULL((
                    SELECT SUM(A.CarteraInicial) 
                    FROM dm_productividad.dbo.fct_stock_manubl_asesor_month A 
                    INNER JOIN CTE_Asesores_Com DA ON A.IdSAsesor = DA.IdSAsesor
                    WHERE DA.IdSAgencia = M.IdSAgencia AND A.Periodo = @periodo
                ), 0) AS CarteraInicial
            FROM dm_productividad.dbo.fct_stock_manubl_agencia_month M 
            WHERE M.Periodo = @periodo 
        ...
        -- =================================================================
        -- 3. RECUPERADORES (Fórmula DAX aplicada a metas y cartera)
        -- =================================================================
        ...
        SELECT 
            ...
            (MS.MetaMoraCPP * ISNULL(NULLIF(MS.CarteraInicial, 0), ISNULL(S.Cartera, 0))) AS moraCppMax, 
            ISNULL(S.MoraCPP, 0) AS moraCppActual,
            
            (MS.MetaMoraDeficiente * ISNULL(NULLIF(MS.CarteraInicial, 0), ISNULL(S.Cartera, 0))) AS moraDefMax, 
